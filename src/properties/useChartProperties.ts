...
// 分割点属性处理
export function handleSplitPoint(props: RenderPropOptions, options: Array<SeriesOption>): Array<any> {
  const { split } = props.chartOptions || {};
  const { dataItems = [] } = props.dataOptions;
  return options
    .map((item, index) => {
      let splitOptions =
        dataItems.length && dataItems[index].split ? dataItems[index].split : split;
      if (splitOptions) {
        let {
          styleAfter,
          styleBefore,
        ...
        if (!trend) {
          item.data = (item.data as Array<any>).map((da, daIndex) => {
            return {
              value: da,
              ...getValueOptionsByType(item.type, daIndex > splitX ? styleAfter : styleBefore)
            };
          });
          return item;
        }
        ...
        if (["before", "after"].includes(trendPart)) {
          const trendOptions = getTrendOptionsByType(trend, props, trendStyle || dataItems[index]);
          if (trendPart === "before") {
            return [
